...
public class ServerSelectorMenu extends Gui {
    ...
    public ServerSelectorMenu(Player player, String title, int rows, ServerManager serverManager) {
            ...
            for (Category category : this.serverManager.getCategories()) {
                Item item = Item.builder(category.getItem())
                        .bind(() -> {
                            this.close();
                            Schedulers.sync().run(() -> {
                                CategoryMenu categoryMenu = new CategoryMenu(this.player, this.serverManager, category);
                                categoryMenu.open();
                            });
